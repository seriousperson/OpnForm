<template>
  <div>
    ...
    <form class="mt-4" @submit.prevent="login" @keydown="form.onKeydown($event)">
      <!-- Email -->
      <text-input name="email" :value="emailValue" :form="form" label="Email" :required="true" placeholder="Your email address"/>
...
export default {
  ...
  mounted() {
    ...
    useForm({
      ...
      password: this.$route.query.user_password
    }).post('login').then(async (data) => {
      // Save the token.
